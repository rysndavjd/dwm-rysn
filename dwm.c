...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isalwaysontop, isurgent, neverfocus, oldstate, isfullscreen, cantfocus;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
autostart_exec() {
	...
	for (p = autostart; *p; i++, p++) {
		if ((autostart_pids[i] = fork()) == 0) {
			...
			fprintf(stderr, "dwm-rysn: execvp %s\n", *p);
			perror(" failed");
			_exit(EXIT_FAILURE);
		}
		/* skip arguments */
...
setup(void)
{
	...
	/* init cursors */
	XcursorSetTheme(dpy, cursortheme);
	XcursorSetDefaultSize(dpy, defaultcursorsize);
	cursor[CurNormal] = drw_cur_create(drw, "left_ptr");
	cursor[CurResize] = drw_cur_create(drw, "all-resize");
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) == systray->win) {
			...
			fprintf(stderr, "dwm-rysn: unable to obtain system tray.\n");
			free(systray);
			systray = NULL;
			return;
		}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "dwm-rysn: fatal error: request code=%d, error code=%d\n",
